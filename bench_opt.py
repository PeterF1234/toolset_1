    ...
    with open(outfile) as inp:
        ...
        for line in inp:
            if "SPECTRUM VIA TRANSITION ELECTRIC DIPOLE" in line:
                parse = True
            elif "SPECTRUM VIA TRANSITION VELOCITY DIPOLE" in line:
                parse = False
            if parse == True and "Excited State" in line:
                line.strip()
                dat = line.split()
...
def get_errors_hmap(logfile,csvfile,lambdarange=[],ref_pad=50,errorfunc=mse,workdir='.',do_log=True):
    ...
    # SHIFTS = np.arange(70,131,1)/100 # 61 elements
    ...
    for n in SIGMAS:
        ...
        for m in SHIFTS:
            ...
            if current_err < lowest_err and errorfunc != r_square:
                lowest_sigma = n
                lowest_shift = m
                lowest_err = current_err
            elif current_err > highest_r2 and errorfunc == r_square:
                lowest_sigma = n
                lowest_shift = m
                highest_r2 = current_err
        if do_log == True and errorfunc != r_square:
...
def print_result(ref_csv,logfile,lambdarange,SIGMA,lambda_shift,ref_pad=50,dumpfile='foo.txt',writeopt=True,errorfunc=mse,workdir='.',logscale_hmap=True):
    ...
    if writeopt == True:
        ...
        for i in range(len(X_opt)):
            ...
            k.write(optline)
        k.close()
    print("optimized half-width: "+str(lowest_sigma)+" eV")
    print("optimized wavelength shift: "+str(lowest_shift)+" nm")
    print(errorfunc.__name__ + " = "+str(lowest_err)) # összes mse sns heatmap formában?
    plt.plot(X,Y)
